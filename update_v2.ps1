...
foreach ($id in $mapping.Keys) {
    try {
        ...
        $finalExprBlock = $null
        foreach ($op in $json.answerState.proof_step.OPERATIONS) {
            if ($op.type -eq 'conclusion_operation') { $finalExprBlock = $op.inputs.VALUE.block }
        }
        $newOps = New-Object System.Collections.Generic.List[PSObject]
        foreach ($fId in $mapping[$id]) {
            $newOps.Add(@{ type="replace_operation"; inputs=@{ VALUE=@{block=$formulas[$fId].left}; FORMULA=@{type="formula_$fId"}; REPLACEMENT=@{block=$formulas[$fId].right} } })
            $newOps.Add(@{ type="conclusion_operation"; inputs=@{ VALUE=@{block=$formulas[$fId].right} } })
        }
