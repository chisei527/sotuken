...
foreach ($id in 1..10) {
  ...
  # Build expressions (fresh instances per usage)
  $A = TanOf (Pi4)
  ...
  if ($json.PSObject.Properties.Name -contains 'requiredFormulas') { $json.requiredFormulas = $formulaMap[$id] } else { $json | Add-Member -MemberType NoteProperty -Name requiredFormulas -Value $formulaMap[$id] }
  ...
  $hasNull = $txt -match '"block"\s*:\s*null'
  $p = @($parsed.answerState.blocks.blocks | Where-Object { $_.type -eq 'proof_step' })[0]
  $chain = @()
  $cur = $p.inputs.OPERATIONS.block
  while ($cur) {
    $chain += [string]$cur.type
    if ($cur.next -and $cur.next.block) { $cur = $cur.next.block } else { break }
  }
  $okChain = (($chain -join '->') -eq ($expectedChains[$id] -join '->'))
  $okFormula = (($parsed.requiredFormulas -join ',') -eq ($formulaMap[$id] -join ','))
